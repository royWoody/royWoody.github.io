...
jwplayer.version = '6.4.${jwplayer.version}';
	...
	var config = embed.config = function(config) {
		...
		var _defaults = {
			...
			_modes = {
				...
				flash: { type: "flash", src: _config.base + "../Player.swf" }
			};
	...
	embed.download = function(_container, _options, _errorCallback) {
		...
		function _styleElements() {
			...
			_css(_prefix+"logo", {
				...
				background: "top right no-repeat url(" + _logo.file + ")"
			});
...
(function (jwplayer) {
    ...
    var _flash = jwplayer.embed.flash = function (_container, _player, _options, _loader, _api) {
        ...
        this.embed = function () {
            ...
            if (_api.config.seek) {
                utils.log('js seek v0.3:' + _api.config.seek);
                _api.seek(_api.config.seek);
            }
        }
        /**
